...
// Para devolver los valores de los vectores de busqueda a 0
...
// Para devolver los asientos seleccionados a ' ' y 0
...
// toString y toStrings de busqueda especifica y toStringpara despues de comprar
string Cinema::toString() {
	stringstream s;
	s << "CARTELERA: " << endl << endl;
	for (int i = 0; i < cant; i++) {
		for (int j = 0; j < 366; j++) {
			for (int k = 0; k < 3; k++) {
				if (vecSala[i]->getDia(j)->getHora(k)->getFuncion() != NULL) {
					...
					vecPosibleHora[cantPosible] = k; // ingresando el camino de la funcion para poder seleccionar despues
					cantPosible++;
					s << cantPosible << ": " << endl;
...
string Cinema::toStringGenero(string gen) {
	stringstream s;
	s << "CARTELERA POR GENERO: " << gen << endl << endl;
	for (int i = 0; i < cant; i++) {
		for (int j = 0; j < 366; j++) {
			for (int k = 0; k < 3; k++) {
...
string Cinema::toStringNombre(string nom) {
	stringstream s;
	s << "CARTELERA POR NOMBRE: " << nom << endl << endl;
	for (int i = 0; i < cant; i++) {
		for (int j = 0; j < 366; j++) {
			for (int k = 0; k < 3; k++) {
				Hora* temp = vecSala[i]->getDia(j)->getHora(k);
...
string Cinema::toStringDimensiones(int dim) {
	stringstream s;
	s << "CARTELERA POR DIMENSION " << dim << "D" << endl << endl;
	for (int i = 0; i < cant; i++) {
		for (int j = 0; j < 366; j++) {
			for (int k = 0; k < 3; k++) {
				Hora* temp = vecSala[i]->getDia(j)->getHora(k);
...
// Metodo para cambiar de Fecha (mes-dia-annio) a numero de dia
...
// Para ingresar una funcion en una sala, dia y hora especifica
...
// Devuelve el monto que se le cobre por los tiquetes que esta comprando
...
// Confirma asientos seleccionados
...
// Cancela asientos seleccionados
